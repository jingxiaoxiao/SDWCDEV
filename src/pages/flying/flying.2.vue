  ...
  methods: {
    ...
    completeEventHandler(){
      ...
      for(var i = 1; i < pointList.length; i++){
        ...
        var path = [
          new AMap.LngLat(pointList[i-1].lng,pointList[i-1].lat),
          new AMap.LngLat(pointList[i].lng,pointList[i].lat)
        ]
        console.log('线',path)
        // 创建折线实例
        var currentInd
        if(a_mark.lng == pointList[i].lng && a_mark.lat == pointList[i].lat ){
            currentInd = i
        } 
        if(i>currentInd){
